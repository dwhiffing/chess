...
// const DEFAULT_GRID = 'R3K2R/PPPPPPPP/8/8/8/8/pppppppp/r3k2r'
...
export const getPossibleMoves = (grid, tile, meta) => {
  ...
  const possibleMoves = legalMoves.filter((move) => {
    ...
    return !getActiveCheck(newGrid, turnIndex)
  })
...
export const getActiveCheck = (grid, turnIndex) => {
  ...
  return possible.length > 0 ? possible : false
}

export const getActiveCheckmate = (grid, turnIndex) => {
  if (!grid) return
  if (!getActiveCheck(grid, turnIndex)) return false
...
const getSafeMoves = (grid, turnIndex) =>
  grid
    ...
    .filter((move) => {
      const newGrid = getMove(grid, move.from, move.to)
      const stillInCheck = getActiveCheck(newGrid, turnIndex)
      return !stillInCheck
    })
...
const getKingMoves = (grid, sourceTile, castleStatus) => {
  ...
  // moves = moves.concat([i - 2, i + 2])

  // TODO: castling
  //   if there are no pieces between the king and a rook
  //   and neither the king nor rook have moved this game
  //   and the king is not in check
  //   then we may move the king 2 squares toward that rook
  //   and move the rook to the square the king jumped over
